...
```mermaid
flowchart TD
    Init([Инициализация:\ndist[src]=0, остальные = ∞\npred[v] = None\nvisited = ∅]) --> Loop{Есть\nнераскрытые\nвершины?}
    Loop -- Нет --> End([Конец])
    Loop -- Да --> Pick[Выбрать u с min dist[u]\nиз нераскрытых]
    Pick --> Mark[Отметить u как раскрытую]
    Mark --> Relax[Для каждого соседа v у u:\nесли dist[u]+w меньше dist[v]\nобновить dist[v] и pred[v]]
    Relax --> Loop
```
...
```mermaid
flowchart TD
    A([Вычислить входящие степени\nвсех вершин]) --> B[Добавить в очередь\nвсе вершины с in_degree=0]
    B --> C{Очередь\nпуста?}
    C -- Нет --> D[Извлечь u, добавить в порядок]
    D --> E[Для каждого соседа v:\nuменьшить in_degree v на 1\nесли in_degree v = 0 — добавить в очередь]
    E --> C
    C -- Да --> F{len порядка\n= V?}
